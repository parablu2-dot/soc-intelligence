...
요약 스키마 (A-2, 통일 구조화): 축별 1회 호출로 KO 구조화 JSON(executive_summary/key_facts/
implications/counterpoint) 생성 → 같은 배치를 EN으로 번역하는 1회 호출로 미러 생성(scripts/
summary_schema.py 공용). content.{ko,en} 아래만 언어별, sector/date/item_count는 언어 중립.
"""
...
Below are today's collected headlines grouped by sector. For each sector listed, produce a
structured Korean summary via the tool call: executive_summary (2-3 sentences), key_facts
(one line per concrete number/proper noun), implications (keyword+text pairs), counterpoint
(one line caveat, empty string if none).
...
def _generate_sector_summaries(by_axis: dict[str, list[dict]], client, taxonomy: dict) -> dict:
    ...
    tool_schema = content_tool_schema(
        "sector_summaries",
        "Structured competitive-intelligence summary per sector, written in Korean",
        list(active.keys()),
    )
    ...
    en_content = translate_content_batch(
        client, ko_content, "sector_summaries_en", "English translation mirror of sector summaries",
    )

    axis_labels = taxonomy.get("axes", {})
    now_ts = datetime.now(timezone.utc).isoformat()
    today_str = date.today().isoformat()
    return {
        axis: {
            "sector": axis_labels.get(axis, {}).get("label", axis),
            "date": today_str,
            "item_count": len(items),
            "generated_at": now_ts,
            "content": {
                "ko": ko_content.get(axis, {}),
                "en": en_content.get(axis, {}),
            },
        }
        for axis, items in active.items()
    }
