...
요약 스키마 (A-2, 통일 구조화): 그룹(축/카테고리)별 KO 구조화 JSON(executive_summary/key_facts/
implications/counterpoint) 생성 호출 + EN 미러 번역 호출 = 그룹당 2회 (scripts/summary_schema.py 공용).
"""
...
from scripts.summary_schema import CONTENT_PROPERTIES, translate_content_batch
...
_TOOL: dict = {
    ...
    "description": "Structured synthesis of analyst review notes for one axis/category, written in Korean",
    "input_schema": CONTENT_PROPERTIES,
}
...
위 메모들을 종합해 구조화된 요약을 생성하세요: executive_summary(2~3문장), key_facts(메모 속
수치·고유명사 한 줄씩), implications(키워드+설명 쌍), counterpoint(반론/유의점 한 줄, 없으면 빈 문자열).
메모에 없는 사실을 추가하지 마세요. 자료가 부족한 필드는 빈 배열/빈 문자열로 두세요.
한국어로 작성하세요. 이모지 금지.
"""
    ...
    for (axis, category), group in grouped.items():
        ...
        try:
            ...
            en_content = translate_content_batch(
                client, {key: ko_content}, "category_summary_en",
                "English translation mirror of one category summary",
            ).get(key, {})
            ...
            out[key] = {
                ...
                "generated_at": now_ts,
                "content": {"ko": ko_content, "en": en_content},
            }
            ok += 1
